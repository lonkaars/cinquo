...
module.exports.generateTiles = (json, config) => {
	...
	for (let i = 0; i < json.length; i++) {
		...
		$('.panel').addClass(`tile-id-${json[i].id}`);
		if (json[i].display.dark == true)
			$('.panel').addClass('dark');
		...
		if (json[i].type == 'slider') {
			...
			// Slider value
			if (json[i].display.displaySliderValue == true)
				$('.panel').append(
					$('<p></p>')
					.addClass(`value slider-id-${json[i].id}`)
					.text(
						json[i].min != undefined && json[i].max != undefined
							? Math.floor((json[i].min + json[i].max) / 2)
							: 50
					)
				);
		}
		...
		if (size >= 9)
			$('.panel').addClass('large');

		if (json[i].where.w > json[i].where.h)
			$('.panel').addClass('long');
		if (json[i].where.w < json[i].where.h)
			$('.panel').addClass('tall');

		// Background
		switch (json[i].display.type) {
			case "solid": {
